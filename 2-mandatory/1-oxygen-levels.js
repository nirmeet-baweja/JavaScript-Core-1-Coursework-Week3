...
{
  // extract oxygen levels with the "%" symbol
  let numOxygenLevels =
    oxygenLevels.map(level => level.substr(0, level.length-1));

  // Find the very first safe oxygen level
  let safeLevels =
    numOxygenLevels.find(level => (level > 19.5 && level < 23.5));
